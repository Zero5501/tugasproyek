            ...
            <tbody>
              {orderList.map((order) => (
                <tr key={order.idTransaksi}>
                  ...
                  <td>
                  {order.statusTransaksi == 1 ? 
                    <Button variant="primary" onClick={()=>updateStatus(order.idTransaksi, 2)}>Set Paid</Button>
                    : order.statusTransaksi == 2 ? (
                      <Button variant="primary" onClick={()=>updateStatus(order.idTransaksi, 3)}>Set Payment Accepted</Button>
                    ) : order.statusTransaksi ==3 ? (
                      <Button variant="primary" onClick={()=>updateStatus(order.idTransaksi, 4)}>Set Delivered</Button>
                      ): ''}
                  </td>
                </tr>
              ))}
